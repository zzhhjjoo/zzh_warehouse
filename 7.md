...
```text
1.Python的类中__init__方法用于初始化对象的属性。当创建一个新的对象时，__init__方法会自动调用，并允许我们为对象设置初始状态。

2.在Python中，可以通过在子类的定义中将父类作为参数传递来继承父类的属性和方法。要改写（override）父类的方法，只需在子类中定义一个同名的方法即可。
class ParentClass:
    def method(self):
...
child = ChildClass()
child.method()  # Output: This is the child method

3.Python类中有许多特殊的方法，也称为魔术方法或双下划线方法。这些方法允许我们定义类的行为，例如初始化、比较、算术运算等。以下是三个特殊方法的例子：
class MyClass:
...
print(obj1)          # Output: MyClass with value: 5
```

## 实验总结

在这次实验中，我学习了面向对象编程中类的基本概念，包括类的定义、初始化方法（init）、实例方法、继承以及方法的重写。还学习了Python中特殊方法的概念和用法，例如初始化、比较、算术运算等。通过实际的代码示例，我加深了对这些概念的理解。同时，还学习了使用mermaid语法来绘制类图，以便更清晰地展示类之间的关系。
